...
inline void slidepad_hold(slidepad_t *sp, uint8_t x, uint8_t y)
{
    ...
    {
        // 129, 130, ..., 255
        ds4432_set(sp->dac_, DS4432_OUT0, DS4432_SINK, rescale_(x, 129, 255, 1, 127));
    }
    else
    {
        // 127, 126, ..., 0
        ds4432_set(sp->dac_, DS4432_OUT0, DS4432_SOURCE, rescale_(127 - x, 0, 127, 1, 127));
    }
    ...
    {
        // 129, 130, ..., 255
        ds4432_set(sp->dac_, DS4432_OUT1, DS4432_SINK, rescale_(y, 129, 255, 1, 127));
    }
    else
    {
        // 127, 126, ..., 0
        ds4432_set(sp->dac_, DS4432_OUT1, DS4432_SOURCE, rescale_(127 - y, 0, 127, 1, 127));
    }
}
